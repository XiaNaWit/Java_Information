...
HTTP（HyperText Transfer Protocol，超文本传输协议）是一个**无状态**的协议，服务器不保留与客户端交易状态的记录。这意味着每个请求都是独立的，需要通过 Cookie/Session 等机制来维持状态。
...
-   **101 Switching Protocols**  ：服务器根据客户端的请求切换协议，常用于 WebSocket 升级。
...
-   **405 Method Not Allowed**  ：请求方法不被服务器支持，响应中应包含 Allow 首部说明支持的方法。
...
-   **504 Gateway Timeout**  ：充当网关或代理的服务器未及时从上游服务器收到响应。
...
通过使用 **证书（CA Certificate）** 来对通信方进行认证。
...
|:---|:---|
| 公钥 | 网站的公钥，用于加密 |
| 签名 | CA 机构对证书明文数据的数字签名 |
| 颁发者信息 | 签发该证书的 CA 机构信息 |
| 有效期 | 证书的有效起止时间 |
| 主体信息 | 证书持有者（网站）的身份信息 |

#### 数字签名的生成与校验

**生成签名**：CA 机构对证书明文数据 T 做 Hash → 消息摘要，再用 CA 的私钥加密 → 数字签名 S。明文 T 和签名 S 共同组成数字证书。

**校验签名**：客户端用浏览器内置的 CA 公钥对 S 解密 → 消息摘要 S'；对明文 T 做 Hash → T'；若 S' == T'，则证书可信。

#### 证书链

实际场景中，服务器返回的证书一般不是根证书签发的，而是由**中间证书**签发，形成多级信任链。以 baidu.com 为例：

1. 客户端收到 baidu.com 证书，发现签发者不是根证书，于是向 CA 请求中间证书 "GlobalSign Organization Validation CA - SHA256 - G2"
2. 该中间证书由 "GlobalSign Root CA" 签发，而根证书已预装在浏览器信任列表中。用根证书公钥验证中间证书通过后，即信任中间证书
3. 再用中间证书的公钥验证 baidu.com 证书的可信性

> 采用证书链而非根证书直接签发，是为了保护根证书的绝对安全。一旦根证书失守，整个信任链都将崩溃。

#### HTTPS 握手过程

1. 客户端请求服务器，建立 TCP 连接
2. 服务器返回 CA 证书（含网站信息、公钥）
3. 客户端验证证书：
   - 用 CA 公钥解密签名 → 消息摘要 S'
   - 对明文做 Hash → T'
   - 对比 S' 和 T'，一致则证书可信
4. 取出公钥，生成随机对称密钥 KEY，用公钥加密后传给服务器
5. 服务器用私钥解密得到 KEY
6. 后续通信使用 KEY 进行对称加密
