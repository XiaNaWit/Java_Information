...
- **TreeSet**：
  - 基于红黑树实现，支持有序性操作，元素不能为 null;
  - 例如根据一个范围查找元素的操作。但是查找效率不如 HashSet，HashSet 查找的时间复杂度为 O(1)，TreeSet 则为 O(logN)。

- **HashSet**：
  - 底层数据结构是哈希表 + 红黑树，元素无序，可为 null，支持快速查找，但不支持有序性操作。
  - 失去了元素的插入顺序信息，也就是说使用 Iterator 遍历 HashSet 得到的结果是不确定的。

- **LinkedHashSet**：
  - 底层数据结构由哈希表和双向链表组成， 父类为 HashSet ，具有其父类的查找效率，并且内部使用双向链表维护元素的插入顺序，元素不可为 null。
...
- **ArrayList**：
    - 非线程安全 
    - 基于动态数组实现，支持随机访问。
    - 持随机存取，在指定的位置插入、删除效率低（因为要移动数组元素）；
    - 底层结构是数组，初始容量为10，如果内部数组容量不足则自动扩容，每次增长1.5倍，因此当数组很大时，效率较低。扩容也是调用Arrays.copyOf方法
...
- **Vector**：
    ...
    - Stack
      - Stack 是 Vector 的一个子类，本质也是一个动态数组结构，不同的是，其数据结构是先进后出，取名叫栈！

- **LinkedList**：
    - 基于双向链表实现，只能顺序访问，但是可以快速地在链表中间插入和删除元素。
    - 实现了Deque接口，因此可以像操作栈、队列和双向队列一样操作它。
...
- **TreeMap**：
  - 底层是红黑树，保证了时间复杂度为 log(n)
  - 可以对其进行排序，使用 Comparator 或者 Comparable
  - 只要 compare 或者 CompareTo 认定该元素相等，那就相等
  - 非同步
  - 自然排序(手动排序)，元素不能为null
    > **复杂度**
    >1. 增删查改时间复杂度均为 O(logN)
    >2. containsKey() 时间复杂度 O(logN)
    >3. containsValue() 时间复杂度 O(N)，因为 value 是无序的，所以要依次遍历


- **HashMap**：
  - 基于哈希表实现，存储的结构是key-value键值对，不像Collection是单列集合
  - k和v允许为null，存储无序
  - 非同步
      - 它是非线程安全的，如果在多线程环境下使用，可能因程序自动扩容操作将单向链表转变成了循环链表，在查询遍历元素的时候，造成程序死循环！
  ...
  - 底层是散列表+红黑树。初始容量为16，装载因子为0.75，每次扩容2倍
      - 为什么是0.75，时间和空间的权衡，如果太小比如0.5那么空间为一半的时候就发生扩容，浪费空间，太大如1，满了才扩容，hash冲突的概率大大增加，

- **HashTable**：
  - 和 HashMap 类似，但它是方法加锁，使用 synchronized 线程安全的，这意味着同一时刻多个线程同时写入 HashTable 不会导致数据不一致。
...
- **LinkedHashMap**：
  ...
  - 大多使用 HashMap 的 API ，只不过在内部重写了某些方法，维护了双向链表
    > **复杂度**
    >1. 增删查改时间复杂度均为O(1)
    >2. containsKey()时间复杂度是O(1)
    >3. containsValue()时间复杂度是O(N)

更多 map 详解：[map源码解读](map源码解读)
