...
Java 对动态绑定的实现主要依赖**方法表**，继承与接口的实现略有不同：

**继承体系**：执行方法时，在方法区找到该类的方法表，确认方法在方法表中的偏移量。若该方法被重写则直接调用子类版本，否则按继承关系搜索父类方法表中该偏移量的方法。

**接口体系**：由于一个类可实现多个接口，同一接口方法在不同类的方法表中位置可能不同，因此不能通过固定偏移量查找，需搜索完整的方法表。

## super 关键字

- `super()`：访问父类构造函数，委托父类完成初始化
- `super.method()`：子类重写父类方法后，可通过 super 引用父类方法实现
