...
虚拟线程 = Continuation（任务包装器）+ Scheduler（执行器）

```java
virtual thread = continuation + scheduler
```

虚拟线程把任务（Runnable）包装到 Continuation 实例中：
- 任务需要阻塞时，调用 Continuation.yield() 让出控制权
- 任务需要继续执行时，Continuation 被恢复执行

Scheduler 是一个 `java.util.concurrent.Executor`，默认使用 FIFO 模式的 ForkJoinPool。

### mount 与 unmount

| 操作 | 说明 |
|:---|:---|
| **mount（挂载）** | 虚拟线程分配给平台线程执行，Continuation 栈数据帧拷贝到平台线程栈（堆→栈） |
| **unmount（卸载）** | 虚拟线程暂停，Continuation 栈数据帧留在堆内存中（栈→堆） |
...
1. 调度器中的平台线程等待处理任务
2. 虚拟线程被分配一个平台线程（载体线程）
3. 虚拟线程运行其 Continuation，执行用户任务
4. 任务执行完成 → Continuation 终结 → 虚拟线程终结 → 平台线程返还调度器

### Continuation yield 实验

Continuation 被封闭在 `java.base/jdk.internal.vm` 下，通过编译参数 `--add-exports java.base/jdk.internal.vm=ALL-UNNAMED` 暴露：
...
yield 调用后再次调用 run 可从 yield 处继续执行，实现中断和恢复。
